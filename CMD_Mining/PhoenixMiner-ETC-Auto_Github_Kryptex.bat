...
:: 设置控制台窗口标题
title PhoenixMiner 6.2c Auto_Mining

:: 显示脚本标题和信息
echo.
echo ========================================
echo        PhoenixMiner Auto_Download&Mining
echo ========================================
echo PhoenixMiner版本: 6.2c
echo ========================================
echo.
...
:: 设置下载URL和文件名变量
set "FILENAME=PhoenixMiner_%VERSION%_Windows.zip"	
set "DOWNLOAD_URL=https://github.com/acu715/acu715/raw/refs/heads/main/Mining/PhoenixMiner_6.2/%FILENAME%"

:: 检查文件是否已存在且大小正确
if exist "%FILENAME%" (
    echo [信息] 发现已存在的PhoenixMiner压缩包，检查文件大小...
    
    :: 获取文件大小（字节）
    for %%I in ("%FILENAME%") do set "FILESIZE=%%~zI"
    
    :: 检查文件大小是否在合理范围内（约4.92MB = 5,166,573字节）
    :: 允许一定范围的误差（4.8MB - 5MB）
    if !FILESIZE! GEQ 5033164 if !FILESIZE! LEQ 5242880 (
        echo [信息] 文件大小正确（约!FILESIZE!字节），跳过下载。
        goto :EXTRACT_PhoenixMiner
    ) else (
        echo [警告] 文件大小不正确（!FILESIZE!字节），可能需要重新下载。
        echo        预期大小：约4.92MB（5,166,573字节）
        echo.
...
:: 检查文件是否已存在
if exist "%FILENAME%" (
    echo [信息] 发现已存在的PhoenixMiner压缩包，跳过下载。
    goto :EXTRACT_PhoenixMiner
)

:: 下载PhoenixMiner
echo [信息] 正在下载PhoenixMiner: %FILENAME%
...
:EXTRACT_PhoenixMiner
:: 解压文件
echo [信息] 正在解压文件...
set "DIRNAME=PhoenixMiner_%VERSION%_Windows"
...
if exist "%DIRNAME%" (
    cd "%DIRNAME%"
    echo [成功] 已进入PhoenixMiner目录。
    echo.
    echo ========================================
    echo            PhoenixMiner 配置信息
    echo ========================================
    echo 矿池地址: etc.kryptex.network:8033
    echo 用户: qiuzhaojinting@gmail.com/%RIG_NAME%
    echo ========================================
    echo.
    echo [信息] 正在启动PhoenixMiner...
    echo        按Ctrl+C可停止程序。
    echo.
    
    :: 运行Rigel
    PhoenixMiner.exe -coin etc -pool ssl://etc.kryptex.network:8033 -wal qiuzhaojinting@gmail.com/%RIG_NAME%
) else (
