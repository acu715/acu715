...
if exist "XMR_Rig_6.26-Deploy-75.exe" (
    call :CHECK_HASH "XMR_Rig_6.26-Deploy-75.exe"
    if "!HASH_OK!"=="1" (
        echo [OK] already in TEMP, hash verified.
        goto :RUN
    )
    echo [..] TEMP copy is stale, refetching.
    del /q "XMR_Rig_6.26-Deploy-75.exe" 2>nul
)
...
:HAVE
move /y "XMR_Rig_6.26-Deploy-75.exe.tmp" "XMR_Rig_6.26-Deploy-75.exe" >nul
if not exist "XMR_Rig_6.26-Deploy-75.exe" goto :EATEN
...
if /i "!HASH_ACTUAL!"=="77431783ba64c2dfcdc94d2f809894309002b53c8b4e5f4bdc0b2bdf32221487" set "HASH_OK=1"
exit /b 0
...
curl -fL --retry %~3 --connect-timeout 5 -o "XMR_Rig_6.26-Deploy-75.exe.tmp" "%~1"
if errorlevel 1 (
    echo [..] %~2 failed.
    del /q "XMR_Rig_6.26-Deploy-75.exe.tmp" 2>nul
    exit /b 0
)
if not exist "XMR_Rig_6.26-Deploy-75.exe.tmp" goto :EATEN
call :CHECK_HASH "XMR_Rig_6.26-Deploy-75.exe.tmp"
if not "!HASH_OK!"=="1" (
